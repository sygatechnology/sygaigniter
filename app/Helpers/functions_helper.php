...
function plugin_helper($plugin_name, $filename)
{
    $filenames = (array) $filename;
    $helpers = [];
    foreach ($filenames as $filename) {
        $helpers[] = 'Plugin\\' . $plugin_name . '\Helpers\\' . $filename;
    }
    helper($helpers);
}
...
if (!function_exists('slugify')) {
    function slugify(string $string, string $separator = "-"): string
    {
        helper('text');
        helper('inflector');
        $string = strtolower(convert_accented_characters($string));
        if ($separator == '_') {
            return underscore($string);
        }
        return dasherize_spaces($string);
    }
}

if (!function_exists('isSerialized')) {
    function isSerialized($input): bool
    {
        if (!is_string($input)) {
            return false;
        }
...
if (!function_exists('serialize_args')) {
    function serialize_args($input): string
    {
        return addslashes(serialize($input));
    }
}

if (!function_exists('unserialize_args')) {
    function unserialize_args($input, $toObject = false)
    {
        $data = unserialize(stripslashes($input));
        if ($toObject !== false) {
            $data = (object) $data;
        }
...
if (!function_exists('chmod_r')) {
    function chmod_r($path, $perm = 0777)
    {
        exec("chmod $perm -R $path");
    }
}
