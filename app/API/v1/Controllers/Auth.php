...
class Auth extends BaseController
{
    public function authenticate()
    {
        ...
        //}
        //return $this->respond(['Token required'], 400, 'no_token');
    }
    ...
    public function activeAccount()
    {
        ...
        if ($token === null) {
            return $this->respond(['User token required'], 400);
        }
        $db = \Config\Database::connect();
        $row = $db->table('users')->where('reset_psswd_token', $token)->get()->getFirstRow();
        if ($row === null) {
            return $this->respond(['User token not found'], 404);
        }
        unset($token);
        $row = (object) $row;
        helper('security');
    ...
    public function logout()
    {
        $auth = \Config\Services::auth();
        if (!$auth->isLoggedIn()) {
            return $this->respond(['User session not found'], 406);
        }
